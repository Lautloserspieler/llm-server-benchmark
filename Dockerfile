...
ARG LLMBENCH_CUDA_ARCHITECTURES=89;120
...
RUN cmake -S /src/llama.cpp -B /src/llama.cpp/build -G Ninja \
      ...
      -DGGML_CUDA=ON \
      -DCMAKE_CUDA_ARCHITECTURES="${LLMBENCH_CUDA_ARCHITECTURES}" \
      -DLLAMA_CURL=OFF \
    ...
    && ldd /src/llama.cpp/build/bin/llama-bench | grep -q 'libcuda.so.1'
...
LABEL org.opencontainers.image.title="llm-server-benchmark" \
      ...
      io.llmbench.cuda-architectures="${LLMBENCH_CUDA_ARCHITECTURES}" \
