...
def _powershell_download(url: str, target: Path, timeout: int) -> None:
    """Download using Windows/PowerShell so the Windows trust store is used.

    This is important on company networks where a TLS inspection proxy installs
    its CA in the Windows certificate store but Python's OpenSSL does not know it.
    Certificate verification stays enabled; we never use an insecure SSL context.
    """
    if os.name != "nt":
        raise RuntimeError("PowerShell TLS fallback is only available on Windows.")

    ps = shutil.which("powershell.exe") or shutil.which("powershell")
    if not ps:
        raise RuntimeError("PowerShell wurde fuer den Windows-TLS-Fallback nicht gefunden.")
    ...
    command = (
        ...
        f"-OutFile '{escaped_target}' -TimeoutSec {max(timeout, 30)}"
    )
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"Windows-TLS-Download fehlgeschlagen: {detail}")


def _download_bytes_windows(url: str, timeout: int) -> bytes:
    fd, name = tempfile.mkstemp(prefix="llmbench-web-", suffix=".tmp")
    os.close(fd)
    tmp = Path(name)
    try:
        tmp.unlink(missing_ok=True)
        _powershell_download(url, tmp, timeout)
...
def get_url(url: str, timeout: int = 30) -> bytes:
    ...
    try:
        ...
        if os.name == "nt" and _is_certificate_error(exc):
            print("Python konnte die Zertifikatskette nicht pruefen; verwende sicheren Windows-TLS-Fallback...")
            return _download_bytes_windows(url, timeout)
        raise
...
def resolve_latest_ref() -> str:
    try:
        ...
        print(f"Hinweis: Neueste llama.cpp-Version konnte nicht ermittelt werden: {exc}")
    return "master"
...
def resolve_ref(explicit_ref: str | None, pin_file: Path, runtime: Path, source_root: Path, force: bool) -> str:
    if explicit_ref:
    ...
    if not force:
        ...
        if ref_file.is_file():
            ...
            previous_path = path_file.read_text(encoding="utf-8-sig", errors="ignore").strip() if path_file.is_file() else ""
...
def download_zip(url: str, target: Path) -> None:
    ...
    try:
        ...
        tmp.unlink(missing_ok=True)
        if os.name == "nt" and _is_certificate_error(exc):
            print("Python vertraut der lokalen TLS-Zertifikatskette nicht.")
            print("Wechsle automatisch auf Windows-Zertifikatsspeicher (PowerShell)...")
            _powershell_download(url, tmp, 120)
        else:
            raise
    if not tmp.is_file() or not zipfile.is_zipfile(tmp):
        tmp.unlink(missing_ok=True)
